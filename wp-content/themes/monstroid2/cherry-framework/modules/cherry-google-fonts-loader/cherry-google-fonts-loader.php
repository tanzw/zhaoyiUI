	...
	class Cherry_Google_Fonts_Loader {
		...
		public function get_setting( $name ) {
			...
			if ( 'theme_mod' == $type ) {
				...
				return $this->filter_setting( $setting, $name );
			}
			...
			if ( ! empty( $settings ) && isset( $settings[ $name ] ) ) {
				return $this->filter_setting( $settings[ $name ], $name );
			}

			return $this->filter_setting( false, $name );

		}

		/**
		 * Filter fonts settings before applying
		 *
		 * @param  mixed  $value Setting value.
		 * @param  string $name  Setting name.
		 * @return mixed
		 */
		public function filter_setting( $value, $name ) {

			/**
			 * Filter fonts settings before applying
			 */
			return apply_filters( 'cherry_google_fonts_setting_value', $value, $name );
		}
